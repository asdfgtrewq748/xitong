    ...
    <div class="page-content">
      <aside class="sidebar">
        ...
        <el-card shadow="never" style="margin-top: 16px;">
          ...
          <chart-settings-panel :config="config" chart-type="surface" @apply="onSettingsApply" compact />
        </el-card>
      </aside>
      ...
      <main class="chart-area">
        ...
        <div class="chart-container">
          ...
          <Surface3D v-else ref="chartRef" :data="chartData" :config="chartConfig" :height="500" />
        </div>
      </main>
...
import { PieChart, QuestionFilled, RefreshLeft, Download } from '@element-plus/icons-vue'
...
const onImported = ({ rows, columns, datasetId }) => {
  ...
  if (numFields.length >= 3) {
    ...
    config.zField = numFields[2].name
  }
  updateChart()
...
const exportChart = (format) => {
  chartRef.value?.exportChart?.({ type: format, filename: `surface3d_${Date.now()}` })
