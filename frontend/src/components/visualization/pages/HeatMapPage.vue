    ...
    <div class="page-content">
      <aside class="sidebar">
        ...
        <el-card shadow="never" style="margin-top: 16px;">
          ...
          <chart-settings-panel :config="config" chart-type="heatmap" @apply="onSettingsApply" compact />
        </el-card>
      </aside>
      ...
      <main class="chart-area">
        ...
        <div class="chart-container">
          ...
          <heat-map v-else ref="chartRef" :data="chartData" :config="chartConfig" :height="500" />
        </div>
      </main>
...
import { Grid, QuestionFilled, RefreshLeft, Download } from '@element-plus/icons-vue'
...
const onImported = ({ rows, columns, datasetId }) => {
  ...
  if (columns.length >= 3) {
    ...
    config.yField = columns[1].name
    config.colorField = columns.filter(c => c.type === 'number')[0]?.name
  }
  updateChart()
...
const exportChart = (format) => {
  chartRef.value?.exportChart?.({ type: format, filename: `heatmap_${Date.now()}` })
