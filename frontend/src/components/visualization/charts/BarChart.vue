...
import { ChartWrapper, generateBarOption } from '../../../utils/chartWrapper'
...
const updateChart = () => {
  ...
  const hasValidData = props.data.data && (
    ...
    (typeof props.data.data === 'object' && Object.keys(props.data.data).length > 0)
  )
  ...
  const option = generateBarOption(props.data, props.config)
