# 🚀 服务器部署指南 - 巷道支护计算功能更新

## 📋 本次更新内容

### 新增功能
1. **巷道支护计算模块**
   - 完整的巷道支护计算 API
   - 单次计算和批量计算
   - Excel 导入导出功能

2. **公式修正**
   - ✅ hat（压力拱高度）公式：添加 f_top 参数
   - ✅ 等效圆半径公式：r = √(a² + b²)
   - ✅ 锚索锚固长度公式：Lm = Q / (2πRc₀)

3. **前端界面**
   - 巷道支护计算页面
   - 批量队列管理
   - 历史记录
   - 统计分析
   - 结果对比

### 修改的文件
```
backend/
  ├── tunnel_support.py         # 核心计算模块（新增+修改）
  ├── server.py                 # API 接口（添加 f_top 参数）
  └── requirements.txt          # 依赖已包含（无需修改）

frontend/
  └── src/components/
      └── TunnelSupportView.vue # 巷道支护界面（新增+修改）
```

---

## ✅ Docker 文件检查

### 当前 Docker 配置状态

1. **backend/Dockerfile** ✅
   - Python 3.11-slim
   - 所有依赖已在 requirements.txt 中
   - 无需修改

2. **frontend/Dockerfile** ✅
   - Node 18-alpine 构建
   - Nginx 1.25-alpine 运行
   - 无需修改

3. **docker-compose.yml** ✅
   - 后端端口：8000
   - 前端端口：80
   - 数据卷挂载正确
   - 无需修改

### 结论
**✅ 所有 Docker 文件都不需要修改！**

---

## 🚢 部署步骤

### 方法 1: 完整重新部署（推荐）

```bash
# 1. 停止现有容器
docker-compose down

# 2. 拉取最新代码
git pull origin master

# 3. 重新构建镜像（强制不使用缓存）
docker-compose build --no-cache

# 4. 启动服务
docker-compose up -d

# 5. 查看日志
docker-compose logs -f
```

### 方法 2: 快速更新（使用缓存）

```bash
# 1. 拉取最新代码
git pull origin master

# 2. 重新构建（使用缓存）
docker-compose build

# 3. 重启服务
docker-compose restart

# 4. 验证状态
docker-compose ps
```

### 方法 3: 仅更新后端（如果只改了后端）

```bash
# 1. 停止后端容器
docker-compose stop backend

# 2. 重新构建后端
docker-compose build backend

# 3. 启动后端
docker-compose up -d backend

# 4. 查看后端日志
docker-compose logs -f backend
```

---

## 🧪 部署后验证

### 1. 检查容器状态
```bash
docker-compose ps
```

应该显示：
```
NAME                  STATUS              PORTS
mining-backend        Up                  0.0.0.0:8000->8000/tcp
mining-frontend       Up                  0.0.0.0:80->80/tcp
```

### 2. 检查后端健康状态
```bash
curl http://localhost:8000/api/health
```

应该返回：
```json
{
  "status": "healthy",
  "message": "系统运行正常",
  ...
}
```

### 3. 测试巷道支护计算 API
```bash
curl -X POST http://localhost:8000/api/tunnel-support/calculate \
  -H "Content-Type: application/json" \
  -d '{
    "B": 4.8,
    "H": 3.2,
    "K": 1.0,
    "depth": 200,
    "gamma": 18.0,
    "C": 0.5,
    "phi": 32.47,
    "f_top": 2.2
  }'
```

应该返回包含 `hat: 1.889` 的计算结果。

### 4. 访问前端页面
打开浏览器访问：
- `http://your-server-ip/`
- 点击"巷道支护计算"菜单
- 输入参数并测试计算功能

### 5. 查看日志
```bash
# 查看所有日志
docker-compose logs

# 只看后端日志
docker-compose logs backend

# 实时查看日志
docker-compose logs -f --tail=100
```

---

## 🔍 调试技巧

### 进入容器内部
```bash
# 进入后端容器
docker exec -it mining-backend /bin/bash

# 进入前端容器
docker exec -it mining-frontend /bin/sh
```

### 检查后端文件
```bash
docker exec mining-backend ls -la /app/
docker exec mining-backend cat /app/tunnel_support.py | head -20
```

### 手动测试 Python 代码
```bash
docker exec -it mining-backend python -c "
from tunnel_support import TunnelSupportCalculator
calc = TunnelSupportCalculator()
result = calc.calculate_complete({
    'B': 4.8, 'H': 3.2, 'K': 1.0, 'depth': 200,
    'gamma': 18.0, 'C': 0.5, 'phi': 32.47, 'f_top': 2.2
})
print('hat =', result['basic']['hat'])
"
```

应该输出：`hat = 1.889`

---

## ⚠️ 常见问题

### 问题 0A: 🚨 数据导入失败 - API 路径重复 (/api/api/...)

**症状：**
```
POST http://39.97.168.66/api/api/raw/import 404 (Not Found)
```

**原因：**
`getApiBase()` 返回 `/api`，但代码中又添加了 `/api` 前缀，导致路径变成 `/api/api/...`。

**解决方案：**

已修复文件：
- ✅ `frontend/src/stores/globalData.js` - 移除重复的 `/api` 前缀
- ✅ `frontend/src/utils/dataService.js` - 移除重复的 `/api` 前缀

**部署修复：**
```bash
cd /path/to/xitong
git pull origin master
docker-compose build --no-cache frontend
docker-compose up -d
```

修复后，URL 将正确变为：
```
✅ http://39.97.168.66/api/raw/import (正确)
❌ http://39.97.168.66/api/api/raw/import (错误)
```

---

### 问题 0B: 🚨 页面无法打开（数据库、地质建模、科研绘图等页面）

**症状：**
- ✅ 首页可以访问
- ✅ 后端 API 正常 (`/api/health` 返回 200)
- ❌ 点击菜单后页面空白或无响应
- ❌ 浏览器控制台显示 404 错误（加载 .js 文件失败）

**原因：**
前端构建时**代码分割失败**，懒加载的组件文件没有生成。

路由配置使用了懒加载：
```javascript
const DatabaseViewerView = () => import(/* webpackChunkName: "database" */ '../components/DatabaseViewerView.vue')
```

但构建后只有 `app.js` 和 `chunk-vendors.js`，缺少 `database.js`、`modeling.js` 等文件。

**解决方案：**

1. **使用自动修复脚本（推荐）：**
```bash
# 在服务器上执行
cd /path/to/xitong
bash scripts/fix_server_pages.sh
```

2. **手动修复：**
```bash
# 停止容器
docker-compose down

# 强制重新构建前端（必须使用 --no-cache）
docker-compose build --no-cache frontend

# 启动服务
docker-compose up -d

# 验证修复
docker exec $(docker-compose ps -q frontend) ls /usr/share/nginx/html/js/
# 应该看到: database.xxx.js, modeling.xxx.js, visualization.xxx.js 等文件
```

3. **验证修复是否成功：**
```bash
# 进入前端容器
docker exec -it xitong-frontend-1 sh

# 查看 js 文件列表
ls -lh /usr/share/nginx/html/js/

# 应该包含这些文件：
# - app.xxxxx.js
# - chunk-vendors.xxxxx.js
# - chunk-echarts.xxxxx.js
# - chunk-element-plus.xxxxx.js
# - database.xxxxx.js          ← 关键
# - modeling.xxxxx.js          ← 关键
# - visualization.xxxxx.js     ← 关键
# - key-stratum.xxxxx.js
# - borehole.xxxxx.js
# - tunnel-support.xxxxx.js
```

4. **测试页面访问：**
- http://39.97.168.66/database-viewer
- http://39.97.168.66/geological-modeling
- http://39.97.168.66/visualization

**详细修复文档：** 参见 `服务器页面无法打开修复指南.md`

---

### 问题 1: 容器启动失败
```bash
# 查看详细错误
docker-compose logs backend

# 可能原因：
# - 端口被占用（8000 或 80）
# - 依赖安装失败
# - 配置文件错误
```

### 问题 2: f_top 参数未生效
```bash
# 检查后端代码
docker exec mining-backend grep -n "f_top" /app/server.py
docker exec mining-backend grep -n "f_top" /app/tunnel_support.py

# 应该能找到相关代码
```

### 问题 3: 前端无法访问后端
```bash
# 检查网络连接
docker network ls
docker network inspect xitong_default

# 检查后端是否监听正确端口
docker exec mining-backend netstat -tlnp | grep 8000
```

### 问题 4: 计算结果不对
```bash
# 查看后端日志中的调试信息
docker-compose logs backend | grep "DEBUG"

# 应该能看到：
# [DEBUG] 接收到的参数: {...}
# [DEBUG] f_top 值: 2.2
# [DEBUG] 计算结果 hat: 1.889
```

---

## 🔄 回滚方案

如果新版本有问题，可以快速回滚：

```bash
# 1. 回到之前的 git 版本
git log --oneline -10  # 查看历史提交
git checkout <之前的commit-hash>

# 2. 重新构建
docker-compose build

# 3. 重启服务
docker-compose up -d

# 或者直接使用之前的镜像（如果有tag）
docker-compose down
docker tag mining-system-backend:latest mining-system-backend:backup
# ... 然后恢复旧镜像
```

---

## 📊 性能优化建议

### 1. 启用 Docker BuildKit
```bash
# 在构建时使用 BuildKit
DOCKER_BUILDKIT=1 docker-compose build
```

### 2. 配置资源限制（可选）
在 `docker-compose.yml` 中添加：
```yaml
services:
  backend:
    deploy:
      resources:
        limits:
          cpus: '2'
          memory: 2G
        reservations:
          cpus: '1'
          memory: 1G
```

### 3. 启用日志轮转
```yaml
services:
  backend:
    logging:
      driver: "json-file"
      options:
        max-size: "10m"
        max-file: "3"
```

---

## ✅ 部署检查清单

部署前：
- [ ] 代码已提交到 git
- [ ] 本地测试通过
- [ ] 备份当前运行的容器

部署中：
- [ ] 停止旧容器
- [ ] 拉取最新代码
- [ ] 构建新镜像
- [ ] 启动新容器

部署后：
- [ ] 容器状态正常
- [ ] 健康检查通过
- [ ] API 测试通过
- [ ] 前端页面可访问
- [ ] 巷道支护计算功能测试
- [ ] 日志无错误

---

## 📞 支持

如果遇到问题：
1. 查看日志：`docker-compose logs`
2. 检查容器状态：`docker-compose ps`
3. 进入容器调试：`docker exec -it mining-backend /bin/bash`

祝部署顺利！🎉
